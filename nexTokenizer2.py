...
def tokenizeScript(script:str, scriptName:str = "Unknown Nexus Module") -> object:
  ...
  def findNextReservedSingleCharToken(searchToken:str = None) -> int:
    ...
    while True:
      if cursor < scriptLen:
        ...
        if not searchToken:
          if script[cursor] in allReservedTokens:
            break # found match
          else: cursor += 1
  ...
  def getToken() -> str:
    ...
    elif aToken in allReservedTokens:
    ...
    if currentToken is None:
      ...
      # the currentToken is a string delimiter
        ...
        if processingStr:
          ...
          if isEscaped:
            tokenStack.insert(tokenLineNumber, allReservedTokens[currentToken], currentToken)
      ...
      elif currentToken in allReservedTokens:
